...
function insertQuizReview($mysqli) {
    $Title = $_POST['Title'];
    $Rating = $_POST['Rating'];
    $ReviewComment = $_POST['ReviewComment'];
    
    $stmt = $mysqli->stmt_init();
    if ($stmt = $mysqli->prepare("INSERT INTO `quizReview` (`id`, `persons_id`, `quizzes_id`, `title`, "
            . "`review`, `date_submitted`, `rating`) VALUES (NULL, '" . $_SESSION['PersonID'] . "', '" . $_SESSION['QuizID'] . "', '" 
...
function updateLessonReview($mysqli) {
    ...
    $stmt = $mysqli->stmt_init();
    if ($stmt = $mysqli->prepare("UPDATE  `quizReview` SET  `title` =  '$Title', `rating` =  '$Rating', `review` = '$ReviewComment' WHERE  `quizzes_id` = " . $_SESSION['QuizID'])) {
...
function DeletequizReview($mysqli) {
    $Title = $_POST['Title'];
    $Rating = $_POST['Rating'];
    $ReviewComment = $_POST['ReviewComment'];
    
    $stmt = $mysqli->stmt_init();
    if ($stmt = $mysqli->prepare("DELETE FROM `quizReview` WHERE `quizzes_id` = " . $_SESSION['QuizID'])) {
